...
class gdrive_main():
    ...
    def download_file(self,filename):
        ...
        with open('/www/wwwroot/{}'.format(filename),'wb') as fh:
    ...
    def download_file_e(self,filename):
        file_id = self._get_file_id(filename)
        service = build('drive', 'v3', credentials=self.__creds)
        request = service.files().get_media(fileId=file_id, acknowledgeAbuse=1)
        with open('/www/wwwroot/{}'.format(filename),'wb') as fh:
            downloader = MediaIoBaseDownload(fh, request)
            done = False
            while done is False:
...
if __name__ == "__main__":
    ...
    # file1 = open('/www/wwwroot/download_files.txt', 'r', encoding='utf-8')
    ...
    try:
        q.download_file(file)
    except Exception:
        q.download_file_e(file)
